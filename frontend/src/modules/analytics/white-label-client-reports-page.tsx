        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {whiteLabelReports.map((client, i) => (
            ...
            >
              ...
              <div className="p-4 sm:p-app-xl" style={{ borderLeft: `4px solid ${client.primaryColor}` }}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    {/* Logo placeholder */}
                    <div
                      className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[var(--app-radius-lg)] text-white font-bold text-lg"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                  {client.kpis.map((kpi) => (
                    <div
                      key={kpi.label}
                      className={cn(
                        'rounded-[var(--app-radius-lg)] border p-2',
                        'bg-[var(--app-hover-bg)] border-[var(--app-border)]',
                      )}
                    >
                ...
                <div
                  className={cn(
                    'rounded-[var(--app-radius-lg)] border p-3 mb-4',
                    'bg-[var(--app-hover-bg)] border-[var(--app-border)]',
                  )}
                >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {client.accessPermissions.map((perm) => (
                      <span
                        key={perm.email}
                        className={cn(
                          'inline-flex items-center gap-1 rounded-[var(--app-radius-lg)] px-2 py-1 text-[10px]',
                ...
                <div className="flex items-center gap-2 pt-3 border-t" style={{ borderColor: 'var(--app-border)' }}>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className={cn(
                      'flex-1 inline-flex items-center justify-center gap-1.5 rounded-[var(--app-radius-lg)] px-3 py-2 text-xs font-medium transition-colors',
                      isDark
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className={cn(
                      'inline-flex items-center justify-center gap-1.5 rounded-[var(--app-radius-lg)] px-3 py-2 text-xs font-medium transition-colors',
                      isDark
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className={cn(
                      'inline-flex items-center justify-center rounded-[var(--app-radius-lg)] px-3 py-2 text-xs font-medium transition-colors',
                      isDark
                        ? 'bg-white/[0.06] text-zinc-300 hover:bg-white/[0.1]'
                        : 'bg-black/[0.04] text-zinc-600 hover:bg-black/[0.08]',
                    )}
          ...
          <motion.div
            ...
            className={cn(
              'rounded-[var(--app-radius-xl)] border-2 border-dashed p-app-3xl flex flex-col items-center justify-center min-h-[300px] cursor-pointer',
          ...
          >
            <div className={cn('flex h-14 w-14 items-center justify-center rounded-[var(--app-radius-xl)] mb-3', 'bg-[var(--app-hover-bg)]')}>
