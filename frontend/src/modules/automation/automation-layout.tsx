...
import {
  ...
  LogOut, Calendar,
  Home, ArrowLeft, ArrowRight, Zap,
  Workflow, MousePointerClick, Play, GitBranch, Clock,
  LayoutTemplate, Users, DollarSign, Megaphone, Factory,
  UserCheck, BellRing, Webhook, ShieldAlert, Timer, BrainCircuit,
  Plus, TestTube, FileSpreadsheet,
} from 'lucide-react';
...
const navSections: NavSection[] = [
  {
    ...
    items: [
      ...
      { id: 'workflow-builder', label: 'Workflow Builder', icon: Workflow, badge: 'BETA', badgeColor: 'from-blue-500/20 to-cyan-500/20' },
      { id: 'ai-autonomous-workflows', label: 'AI Autonomous', icon: BrainCircuit, badge: 'AI', badgeColor: 'from-violet-500/20 to-purple-500/20' },
    ],
  },
  {
    title: 'Builder',
  ...
  },
  {
    ...
    items: [
      { id: 'notifications', label: 'Notifications', icon: BellRing, badge: '5', badgeColor: 'from-red-500/20 to-orange-500/20' },
      { id: 'webhook-integrations', label: 'Webhooks', icon: Webhook },
      { id: 'sla-escalation', label: 'SLA Escalations', icon: ShieldAlert, badge: '2', badgeColor: 'from-red-500/20 to-orange-500/20' },
...
function PageContent() {
  const { currentPage } = useAutomationStore();
...
export default function AutomationLayout() {
  ...
  const { currentPage, sidebarOpen, setSidebarOpen, goBack, goForward, canGoBack, canGoForward, navigateTo } = useAutomationStore();
  ...
  const currentLabel = allNavItems.find(n => n.id === currentPage)?.label || 'Automation';
        ...
        <header className={cn(
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={closeModule}
                  className={cn('shrink-0 h-8 w-8 rounded-[var(--app-radius-lg)]', isDark ? 'hover:bg-white/[0.06] text-white/50 hover:text-white' : 'hover:bg-black/[0.06] text-black/50 hover:text-black')}
                >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={goBack} disabled={!canBack}
                  className={cn('shrink-0 h-8 w-8 rounded-[var(--app-radius-lg)] transition-opacity', !canBack && 'opacity-30 cursor-not-allowed', canBack && isDark && 'hover:bg-white/[0.06]', canBack && !isDark && 'hover:bg-black/[0.06]')}
                >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={goForward} disabled={!canForward}
                  className={cn('shrink-0 h-8 w-8 rounded-[var(--app-radius-lg)] transition-opacity', !canForward && 'opacity-30 cursor-not-allowed', canForward && isDark && 'hover:bg-white/[0.06]', canForward && !isDark && 'hover:bg-black/[0.06]')}
                >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={cn('hidden md:flex items-center gap-2 px-3 py-1.5 rounded-[var(--app-radius-lg)] border w-64 transition-colors', 'bg-[var(--app-hover-bg)] border-[var(--app-border)]')}>
              ...
              <input type="text" placeholder="Search workflows... (⌘K)" className={cn('bg-transparent text-sm focus:outline-none w-full', 'text-[var(--app-text)] placeholder:text-[var(--app-text-muted)]')} />
            ...
            <Tooltip>
              ...
              <TooltipContent>Date Filters</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={() => navigateTo('workflow-builder')} className="hidden md:flex h-8 w-8 rounded-[var(--app-radius-lg)]">
                  <Plus className="w-4 h-4" />
                </Button>
              </TooltipTrigger>
              <TooltipContent>Quick Create Workflow</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="relative h-8 w-8 rounded-[var(--app-radius-lg)]">
                  ...
                  <span className={cn('absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center', 'bg-[var(--app-card-bg)] text-[var(--app-text)]')}>3</span>
                </Button>
              </TooltipTrigger>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={() => navigateTo('ai-autonomous-workflows')} className="relative hidden md:flex h-8 w-8 rounded-[var(--app-radius-lg)]">
                  <Sparkles className="w-4 h-4" />
                  <motion.div className="absolute inset-0 rounded-[var(--app-radius-lg)]"
                    animate={{ boxShadow: ['0 0 0 0 rgba(139,92,246,0)', '0 0 0 4px rgba(139,92,246,0.1)', '0 0 0 0 rgba(139,92,246,0)'] }}
                    transition={{ duration: 2, repeat: Infinity }}
                  />
                </Button>
              </TooltipTrigger>
              <TooltipContent>AI Automation Assistant</TooltipContent>
            </Tooltip>
            ...
            <div className="relative">
              <Button variant="ghost" size="icon" onClick={() => setShowUserMenu(!showUserMenu)}
                className={cn('h-8 w-8 rounded-[var(--app-radius-lg)] font-bold text-xs', 'bg-[var(--app-card-bg)] text-[var(--app-text)] hover:bg-[var(--app-card-bg-hover)]')}
              >
              ...
              {showUserMenu && (
                ...
                >
                  ...
                  <button onClick={() => { logout(); setShowUserMenu(false); }}
                    className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-[var(--app-radius-lg)] text-sm transition-colors', 'text-[var(--app-text-secondary)] hover:text-[var(--app-text)] hover:bg-[var(--app-hover-bg)]')}
                  >
                          ...
                          return (
                            ...
                            >
                              <item.icon className={cn(
                                ...
                                isActive ? 'text-[var(--app-text)]' : 'text-[var(--app-text-muted)] group-hover:text-[var(--app-text-secondary)]'
                              )} />
                              <span className="truncate">{item.label}</span>
                              {item.badge && (
                                <Badge variant="secondary" className={cn('ml-auto text-[9px] px-1.5 py-0 border-0 bg-gradient-to-r', item.badgeColor || '', 'text-[var(--app-purple)]')}>
                                  {item.badge}
                                </Badge>
                              )}
                            </button>
                ...
                <div className={cn('p-3 border-t space-y-3', 'border-[var(--app-border)]')}>
                  <div className={cn('rounded-[var(--app-radius-lg)] p-3 border', 'bg-[var(--app-hover-bg)] border-[var(--app-border)]')}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-medium">Automation Alert</span>
                    </div>
                    <p className={cn('text-[11px] leading-relaxed', 'text-[var(--app-text-muted)]')}>
                      2 workflows stuck in retry loop. Lead follow-up SLA breach detected. 1 webhook failing since 2 hours.
                    </p>
                  </div>
                </div>
              </motion.aside>
