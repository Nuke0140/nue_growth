  ...
  return (
    <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15}}
      ...
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <div>
          <h3 className={cn('text-sm font-semibold', isDark ? 'text-white' : 'text-zinc-900')}>Financial Impact Summary</h3>
          <p className={cn('text-xs', isDark ? 'text-zinc-400' : 'text-zinc-500')}>Total recovered through automation this quarter</p>
        </div>
        <div className="ml-auto text-right">
          <p className={cn('text-xl font-bold', isDark ? 'text-emerald-400' : 'text-emerald-600')}>₹{(amount / 100000).toFixed(1)}L</p>
          <p className={cn('text-[10px] font-medium', isDark ? 'text-zinc-500' : 'text-zinc-400')}>₹{amount.toLocaleString('en-IN')}</p>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <h1 className={cn('text-2xl font-bold tracking-tight', isDark ? 'text-white' : 'text-zinc-900')}>Finance Automations</h1>
            </div>
            <p className={cn('text-sm mt-1.5 ml-[46px]', isDark ? 'text-zinc-400' : 'text-zinc-500')}>Automate financial workflows and compliance</p>
          </div>
