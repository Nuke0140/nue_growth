...
export default function PayoutsPage() {
  ...
  const kpiStats = [
    { label: 'Pending', value: pendingPayouts.length.toString(), amount: formatINR(pendingAmount), icon: Clock, color: 'text-amber-400', bg: isDark ? 'bg-amber-500/10' : 'bg-amber-50' },
    { label: 'Processing', value: processingPayouts.length.toString(), icon: RefreshCw, color: 'text-sky-400', bg: isDark ? 'bg-sky-500/10' : 'bg-sky-50' },
    { label: 'Completed', value: completedPayouts.length.toString(), icon: CheckCircle2, color: 'text-emerald-400', bg: isDark ? 'bg-emerald-500/10' : 'bg-emerald-50' },
    { label: 'Failed', value: failedPayouts.length.toString(), icon: AlertCircle, color: 'text-red-400', bg: isDark ? 'bg-red-500/10' : 'bg-red-50' },
  ];
    ...
    <div className="h-full overflow-y-auto">
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center', isDark ? 'bg-white/[0.06]' : 'bg-black/[0.06]')}>
              <Send className={cn('w-5 h-5', isDark ? 'text-white/60' : 'text-black/60')} />
            </div>
            <div>
              <h1 className="text-xl md:text-2xl font-bold">Payouts</h1>
              <p className={cn('text-xs', isDark ? 'text-white/30' : 'text-black/30')}>Payout Center</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {kpiStats.map((stat, i) => (
            ...
            >
              ...
              {stat.amount && <p className={cn('text-xs mt-1', isDark ? 'text-white/30' : 'text-black/30')}>{stat.amount}</p>}
            </motion.div>
          ))}
        </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center', isDark ? 'bg-white/[0.06]' : 'bg-black/[0.06]')}>
                        <Send className={cn('w-4 h-4', isDark ? 'text-white/50' : 'text-black/50')} />
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className={cn('text-[10px]', isDark ? 'text-white/30' : 'text-black/30')}>{payout.initiatedDate}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Banknote className={cn('w-4 h-4', isDark ? 'text-white/40' : 'text-black/40')} />
              <span className={cn('text-sm font-semibold', isDark ? 'text-white/70' : 'text-black/70')}>All Payouts</span>
            ...
            {payouts.map((payout: Payout, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn('p-4 rounded-xl border transition-colors cursor-pointer', isDark ? 'border-white/[0.04] hover:bg-white/[0.03]' : 'border-black/[0.04] hover:bg-black/[0.02]', payout.status === 'failed' && (isDark ? 'bg-red-500/[0.02]' : 'bg-red-50/20'))}
                >
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className={cn('w-9 h-9 rounded-lg flex items-center justify-center shrink-0', isDark ? 'bg-white/[0.06]' : 'bg-black/[0.06]')}>
                  ...
                  {payout.utrNumber && (
                    <div className="mt-2 ml-12">
                      <span className={cn('text-[10px]', isDark ? 'text-white/30' : 'text-black/30')}>UTR: </span>
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className={cn('text-sm font-semibold', isDark ? 'text-white/70' : 'text-black/70')}>Batch Processing</span>
          </div>
          <div className={cn('flex items-center justify-between p-4 rounded-xl border', isDark ? 'border-dashed border-white/[0.1] bg-white/[0.01]' : 'border-dashed border-black/[0.1] bg-black/[0.01]')}>
            <div className="flex items-center gap-3">
              <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center', isDark ? 'bg-white/[0.06]' : 'bg-black/[0.06]')}>
                <Layers className={cn('w-5 h-5', isDark ? 'text-white/40' : 'text-black/40')} />
              </div>
              <div>
                <p className="text-sm font-medium">Batch Payout Processing</p>
                <p className={cn('text-xs', isDark ? 'text-white/30' : 'text-black/30')}>Select multiple approved payouts for batch processing</p>
              </div>
            </div>
