    ...
    <div className="h-full overflow-y-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={cn('text-xl font-semibold', isDark ? 'text-white' : 'text-gray-900')}>Content Approval</h1>
      ...
      <motion.div
        ...
        className={cn('rounded-2xl border p-4 overflow-x-auto', isDark ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-white border-black/[0.06]')}
      >
          ...
          {PIPELINE_STAGES.map((stage, i) => (
            <div key={stage.stage} className="flex items-center gap-1 flex-1">
              <div className="flex items-center gap-2 flex-1">
                ...
                <span className={cn('text-xs font-medium truncate', isDark ? 'text-white/60' : 'text-gray-600')}>{stage.label}</span>
      ...
      <div className="flex gap-6">
                    ...
                    {items.map((item, i) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <h3 className={cn('text-sm font-medium line-clamp-1', isDark ? 'text-white' : 'text-gray-900')}>{item.title}</h3>
                        </div>
                        <div className="flex items-center gap-1.5 mb-2">
                          ...
                          <span className={cn('text-[10px]', isDark ? 'text-white/30' : 'text-gray-400')}>v{item.version}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className={cn('text-[10px]', isDark ? 'text-white/40' : 'text-gray-500')}>{item.author}</span>
                          <div className="flex items-center gap-2">
                            {item.comments.length > 0 && (
                              <div className="flex items-center gap-1">
                                <MessageSquare className={cn('w-3 h-3', isDark ? 'text-white/30' : 'text-gray-400')} />
                                <span className={cn('text-[10px]', isDark ? 'text-white/30' : 'text-gray-400')}>{item.comments.length}</span>
                              </div>
                            )}
                            <span className={cn('text-[10px]', isDark ? 'text-white/30' : 'text-gray-400')}>{item.createdAt}</span>
                          </div>
                        </div>
                      </motion.div>
                    ))}
              ...
              {filteredItems.map((item, i) => (
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <h3 className={cn('text-sm font-medium mb-1', isDark ? 'text-white' : 'text-gray-900')}>{item.title}</h3>
                      <div className="flex items-center gap-2">
                        <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded-full', isDark ? 'bg-white/[0.08] text-white/50' : 'bg-gray-100 text-gray-600')}
                          style={{ borderLeft: `2px solid ${TYPE_COLORS[item.type] || '#6b7280'}` }}>
                          {item.type}
                        </span>
                        <span className={cn('text-[10px]', isDark ? 'text-white/40' : 'text-gray-500')}>by {item.author}</span>
                        <span className={cn('text-[10px]', isDark ? 'text-white/30' : 'text-gray-400')}>v{item.version}</span>
                        <span className={cn('text-[10px]', isDark ? 'text-white/30' : 'text-gray-400')}>{item.createdAt}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {item.comments.length > 0 && (
                        <div className="flex items-center gap-1">
                          <MessageSquare className={cn('w-3.5 h-3.5', isDark ? 'text-white/30' : 'text-gray-400')} />
                          <span className={cn('text-xs', isDark ? 'text-white/40' : 'text-gray-500')}>{item.comments.length}</span>
                        </div>
                      )}
                      <Eye className={cn('w-4 h-4', isDark ? 'text-white/30' : 'text-gray-400')} />
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <AnimatePresence>
          {selectedItem && (
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className={cn('text-sm font-semibold', isDark ? 'text-white' : 'text-gray-900')}>Content Detail</h3>
              ...
              <div className="flex items-center gap-1 overflow-x-auto pb-1">
                {PIPELINE_STAGES.filter(s => s.stage !== 'revision').map((s, i) => (
                  <div key={s.stage} className="flex items-center gap-1 shrink-0">
                    ...
                    />
                    <span className={cn('text-[9px]', completedStages.has(s.stage) ? (isDark ? 'text-white/60' : 'text-gray-600') : (isDark ? 'text-white/20' : 'text-gray-300'))}>
                      {s.label}
                    </span>
              ...
              <div className="space-y-2">
                <h4 className={cn('text-sm font-medium', isDark ? 'text-white' : 'text-gray-900')}>{selectedItem.title}</h4>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded-full', isDark ? 'bg-white/[0.08] text-white/50' : 'bg-gray-100 text-gray-600')}
                    style={{ borderLeft: `2px solid ${TYPE_COLORS[selectedItem.type] || '#6b7280'}` }}>
                    {selectedItem.type}
                  </span>
                  <span className={cn('text-[10px] px-2 py-0.5 rounded-full', isDark ? 'text-white/40' : 'text-gray-500')}
                ...
                <div className={cn('text-xs space-y-1 pt-1', isDark ? 'text-white/40' : 'text-gray-500')}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {quickStats.map((stat, i) => (
          ...
          >
            ...
            <p className={cn('text-xs mt-1', isDark ? 'text-white/40' : 'text-gray-500')}>{stat.label}</p>
          </motion.div>
        ))}
      </div>
    </div>
