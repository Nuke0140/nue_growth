              ...
              <div key={td.shift.id} className="flex items-center gap-3">
                ...
                <div className={cn('flex-1 h-8 rounded-lg relative overflow-hidden', 'bg-[var(--app-hover-bg)]')}>
                  ...
                  {Array.from({ length: 24 }, (_, h) => h).filter(h => h % 3 === 0).map(h => (
                    <div
                      key={h}
                      className={cn('absolute top-0 h-full w-px', 'bg-[var(--app-hover-bg)]')}
                      style={{ left: `${(h / 24) * 100}%` }}
                    />
                  ))}
                  {/* Shift bar */}
          ...
          <div className="flex mt-1 pl-[160px]">
            <div className="flex-1 relative h-4">
              {Array.from({ length: 9 }, (_, i) => i * 3).map(h => (
                <span
                  key={h}
                  className={cn('absolute text-[9px] -translate-x-1/2', 'text-[var(--app-text-disabled)]')}
                  style={{ left: `${(h / 24) * 100}%` }}
                >
                ...
                <div className="p-5">
                  ...
                  <div className={cn('rounded-xl border p-3 mb-4', isDark ? 'border-white/[0.04] bg-white/[0.02]' : 'border-black/[0.04] bg-black/[0.01]')}>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-center">
                        <p className={cn('text-[10px] mb-0.5', 'text-[var(--app-text-muted)]')}>End</p>
                        <p className="text-lg font-bold">{shift.endTime}</p>
                      </div>
                      <div className="text-center ml-4">
                        <p className={cn('text-[10px] mb-0.5', 'text-[var(--app-text-muted)]')}>Duration</p>
