      ...
      <div className="flex items-center gap-2">
        ...
        <TooltipProvider delayDuration={0}>
          <Tooltip>
            <TooltipTrigger asChild>
              <button className={cn('h-9 px-4 rounded-xl flex items-center gap-2 text-xs font-medium shrink-0', 'bg-[var(--app-card-bg)] text-[var(--app-text)] hover:bg-[var(--app-card-bg-hover)]')}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {paginated.map((sop, idx) => {
            ...
            return (
              <motion.div key={sop.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04, duration: 0.3, ease: [0.22, 1, 0.36, 1] }} className={cn('rounded-2xl border p-4 space-y-3 transition-colors', isDark ? 'bg-white/[0.02] border-white/[0.06] hover:bg-white/[0.04]' : 'bg-white border-black/[0.06] hover:bg-gray-50')}>
                {/* Header */}
                <div className="flex items-start justify-between">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className={cn('w-7 h-7 rounded-lg flex items-center justify-center shrink-0', 'hover:bg-[var(--app-hover-bg)]')}><MoreHorizontal className={cn('w-4 h-4', 'text-[var(--app-text-muted)]')} /></button>
                    </DropdownMenuTrigger>
        ...
        {paginated.length === 0 && (
          <div className={cn('rounded-2xl border py-16 flex flex-col items-center justify-center', 'bg-[var(--app-card-bg)] border-[var(--app-border)]')}>
            <div className={cn('w-14 h-14 rounded-2xl flex items-center justify-center mb-3', 'bg-[var(--app-hover-bg)]')}><BookOpen className={cn('w-6 h-6', 'text-[var(--app-text-disabled)]')} /></div>
