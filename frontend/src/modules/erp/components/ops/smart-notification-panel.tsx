...
const priorityMap: Record<string, { icon: typeof Bell; color: string; bg: string; dotColor: string }> = {
  error: { icon: AlertCircle, color: 'text-red-500 dark:text-red-400', bg: 'bg-red-500/10', dotColor: 'bg-red-500' },
  warning: { icon: AlertTriangle, color: 'text-amber-500 dark:text-amber-400', bg: 'bg-amber-500/10', dotColor: 'bg-amber-500' },
  info: { icon: Info, color: 'text-blue-500 dark:text-blue-400', bg: 'bg-blue-500/10', dotColor: 'bg-blue-500' },
  success: { icon: CheckCircle2, color: 'text-emerald-500 dark:text-emerald-400', bg: 'bg-emerald-500/10', dotColor: 'bg-emerald-500' },
};
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {unreadCount > 0 && (
                  <button
                    onClick={markAllRead}
                    className="text-[11px] font-medium text-[var(--app-accent)] hover:text-[var(--app-accent-hover)] transition-colors px-2 py-1 rounded-lg hover:bg-[var(--app-active-bg)]"
                  >
                    Mark all read
                  </button>
                )}
            ...
            {/* Tabs */}
            <div className="flex items-center gap-1 px-5 py-3 shrink-0" style={{ borderBottom: '1px solid var(--app-border)' }}>
              {tabs.map((tab) => (
                <button
                  key={tab.key}
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 flex items-center justify-center w-6 h-6 rounded-lg text-[var(--app-text-disabled)] hover:text-[var(--app-text-muted)] hover:bg-[var(--app-hover-bg)] transition-all shrink-0 mt-0.5"
                          aria-label="Dismiss"
                        >
            ...
            {/* Footer */}
            <div className="shrink-0 px-5 py-3" style={{ borderTop: '1px solid var(--app-border)' }}>
              <button
                onClick={() => {
                  navigateTo('ai-ops');
                  onClose();
