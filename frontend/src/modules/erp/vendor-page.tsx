...
function getStatusConfig(status: VendorStatus, isDark: boolean) {
  switch (status) {
    ...
    case 'terminated':
      return isDark ? 'bg-zinc-500/15 text-zinc-300 border-zinc-500/20' : 'bg-zinc-50 text-zinc-700 border-zinc-200';
    default:
      return isDark ? 'bg-zinc-500/15 text-zinc-300 border-zinc-500/20' : 'bg-zinc-50 text-zinc-700 border-zinc-200';
  }
}
...
function getTypeBadgeColor(type: string, isDark: boolean) {
  const colors: Record<string, string> = {
    ...
    'Security Audits': isDark ? 'bg-orange-500/15 text-orange-300 border-orange-500/20' : 'bg-orange-50 text-orange-700 border-orange-200',
    ...
    'Legal Services': isDark ? 'bg-amber-500/15 text-amber-300 border-amber-500/20' : 'bg-amber-50 text-amber-700 border-amber-200',
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-2">
          <div className="flex items-center gap-2">
            <div className={cn('flex items-center gap-2 px-3 py-2 rounded-xl border w-full sm:w-64 transition-colors', isDark ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-white border-black/[0.06]')}>
              ...
              <input type="text" placeholder="Search vendors..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} className={cn('bg-transparent text-sm focus:outline-none w-full', isDark ? 'text-white/80 placeholder:text-white/25' : 'text-black/80 placeholder:text-black/25')} />
            </div>
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className={cn('text-[10px] uppercase tracking-wider font-medium', isDark ? 'text-white/30' : 'text-black/30')}>SLA Score</span>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className={cn('text-[10px] uppercase tracking-wider font-medium', isDark ? 'text-white/30' : 'text-black/30')}>Quality Score</span>
              ...
              <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)' }}>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1">
                    <FolderOpen className={cn('w-3 h-3', isDark ? 'text-white/30' : 'text-black/30')} />
                    <span className={cn('text-[11px]', isDark ? 'text-white/50' : 'text-black/50')}>{vendor.linkedProjects.length} projects</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <FileCheck className={cn('w-3 h-3', isDark ? 'text-white/30' : 'text-black/30')} />
                    <span className={cn('text-[11px]', isDark ? 'text-white/50' : 'text-black/50')}>{vendor.complianceDocs.length} docs</span>
                  </div>
                </div>
        ...
        {paginated.length === 0 && (
          <div className={cn('rounded-2xl border py-16 flex flex-col items-center justify-center', isDark ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-white border-black/[0.06]')}>
            ...
            <p className={cn('text-sm font-medium', isDark ? 'text-white/40' : 'text-black/40')}>No vendors found</p>
